...
task('generate:acir', 'Embeds ACIR code into a level contract').setAction(
  async ({}, hre) => {
    try {
      // Compile the circuit
      console.log('Compiling the circuit...');
      execSync('cd circuits && nargo compile && cd ..', { stdio: 'inherit' });

      // Define input and output file paths
      const inputFile = path.join('circuits', 'target', 'circuits.json');
      const outputFile = path.join('circuits', 'target', 'circuits.txt');
      ...
      console.log(
        'Please inject this into the circuit function of your level contract.'
      );
    } catch (error) {
